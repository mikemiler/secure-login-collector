...
(function ($) {
	...
	class BaseAdminDecryption {
		...
		async getFreePrivateKey(entryId) {
			...
			let wrappedDEK, salt, iv, iterations;
			...
			let pbkdf2Iterations = parseInt( iterations || wrappedDEK.kdf_iterations || 0, 10 );
			if ( ! Number.isFinite( pbkdf2Iterations ) || pbkdf2Iterations <= 0) {
		...
		/**
		 * Backwards-compatible alias for premium plugin.
		 *
		 * @param {number} entryId
		 * @returns {Promise<CryptoKey>}
		 */
		async getStandardPrivateKey(entryId) {
			return this.getFreePrivateKey( entryId );
		}
		...
		bindCopyButtons(entryId) {
			...
			$row.find( '.copy-btn' ).off( 'click' ).on(
				'click',
				async function (e) {
					e.preventDefault();
			...
			$row.find( '.toggle-password-btn' ).off( 'click' ).on(
				'click',
				function (e) {
					e.preventDefault();
			...
			$row.find( '.clear-now-btn' ).off( 'click' ).on(
				'click',
				(e) => {
					e.preventDefault();
		 ...
		 */
		async unwrapDEKWithPassword(wrappedDEK, masterPassword, salt, iv, iterations = 100000) {
			// Validate input
			...
			const wrappingKey = await this.derivePBKDF2Key( masterPassword, salt, iterations );
			...
			const dekPEM = new TextDecoder().decode( decryptedDEK );
		...
		async promptMasterPassword() {
			return new Promise(
                ...
                // Toggle password visibility
									$( '#seculoco-toggle-master-password' ).on(
										'click',
										function () {
